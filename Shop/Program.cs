    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (name == "QEL")
            {
                ...
                {
                    // Specific query
                    // foreach (Instrument value in db.Instruments.Where(p => p.Name.StartsWith("b")))

                    // specific query
                    //foreach (Instrument value in db.Instruments.Where(p => p.Name.Equals("violin")).Select(p=>p))

                    // Get all records from a select 
                    foreach (Instrument value in db.Instruments.Select(p => p))
                    { 
